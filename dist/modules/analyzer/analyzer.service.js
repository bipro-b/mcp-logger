...
class AnalyzerService {
    extractImportantLines(logs, maxLines = 100) {
        if (logs.length === 0)
            return [];
        const scored = logs.map((line, index) => ({
            line,
            index,
    ...
    scoreLine(line) {
        return SEVERITY_RULES.reduce((total, { regex, score }) => (regex.test(line) ? total + score : total), 0);
    }
    deduplicate(lines) {
        const seen = new Set();
